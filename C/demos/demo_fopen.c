...
int main() {
    ...
    char* linea = fgets(buffer, sizeof(buffer), archivo);

    // Ahora 'linea' contiene un puntero al string 'buffer', que contiene la primera linea del archivo.
    // El arreglo 'buffer' ha sido modificado, y contiene la primera linea del archivo.

    // En otras palabras, 'linea' posee la misma direccion de memoria que 'buffer'.

    printf("Primera linea del archivo:\n%s\n", buffer);
    ...
    linea = fgets(buffer, sizeof(buffer), archivo);

    printf("Segunda linea del archivo:\n%s\n", buffer);



    // Podemos seguir llamando a 'fgets' hasta que devuelva NULL, lo que significa que no hay más lineas.

    while(fgets(buffer, sizeof(buffer), archivo) != NULL) {
        printf("Siguiente linea del archivo:\n%s\n", buffer);
    }
