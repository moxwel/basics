...
void pushHeap(tHeap* H, tHeapElem item) {
    ...
    while (insert > 1) {
        ...
        // Si el elemento insertado es menor que su padre, hay que intercambiar los nodos.
        if (H->heap[insert] < H->heap[padre]) {
            printf("[pushHeap] El insertado es menor que el padre, hay que subirlo.\n");
            swapElems(H, insert, padre);
        } else {
            printf("[pushHeap] El elemento no es menor que el padre. No hacer nada.\n");
            break;
        }
...
void popHeap(tHeap* H) {
    ...
    while (izq <= H->nElems) {
        ...
        // Si el hijo izquierdo es menor que el hijo derecho (o no hay hijo derecho), hay que revisar el izquierdo
        if ((H->heap[izq] < H->heap[der]) || (der > H->nElems)) {
            printf("[popHeap] El hijo izquierdo es menor que el hijo derecho. Revisar izquierdo...\n");
            // Si el hijo izquierdo es menor que el nodo actual, entonces el hijo izquierdo debe subir.
            if (H->heap[izq] < H->heap[actual]) {
                printf("[popHeap] El hijo izquierdo es menor al actual, hundir actual...\n");
                swapElems(H, izq, actual);
                actual = izq;
            } else {
                printf("[popHeap] El actual ya es menor que su hijo, terminar...\n");
                // Si el hijo ya no es menor que el actual, entonces terminar.
                return;
            }
        }
        // Si el hijo derecho es menor que el hijo izquierdo, hay que revisar el derecho
        else {
            printf("[popHeap] El hijo derecho es menor que el hijo izquierdo. Revisar derecho...\n");
            // Si el hijo derecho es menor que el nodo actual, entonces el hijo derecho debe subir.
            if (H->heap[der] < H->heap[actual]) {
                printf("[popHeap] El hijo derecho es menor al actual, hundir actual...\n");
